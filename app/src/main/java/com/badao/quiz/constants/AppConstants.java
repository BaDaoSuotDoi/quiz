package com.badao.quiz.constants;

public class AppConstants {
    public static final String EXTRAS_REFRESH_BUNDLE = "extrasRefreshBundle";
    public static  final String PROJECT_ID = "projectId";
    public static  final String VIEW_MODE = "viewMode";
    public static  final String HISTORY_PROJECT_ID = "historyProjectId";
    public  static final int PROJECT_ADD = 1;
    public  static final int SCHEDULE_NOTIFICATION = 1;
    public  static final int PROJECT_UPDATE = 2;
    public  static final int QUESTION_TEMP_ADD = 3;
    public  static final int QUESTION_ACTIVE = 1;
    public  static final int QUESTION_DISABLE= 0;
    public  static final int QUESTION_NORMAL_TYPE= 1;
    public  static final int QUESTION_SELECTION_TYPE= 2;
    public  static final int QUESTION_VOCABULARY_TYPE= 3;
    public  static final int QUESTION_TEMP_ID= -1;
    public  static final int TIMER_COUNTDOWN= -1;
    public  static final int PROJECT_PLAY= 1;
    public  static final int PROJECT_SHOW_ANSWER= 2;
    public  static final int QUESTION_ANSWER_CORRECT= 1;
    public  static final int QUESTION_ANSWER_WRONG= 2;
    public  static final int QUESTION_NO_ANSWER= 0;
    public  static final int USER_CHANGE_ANSWER= 0;
    public  static final String FORMAT_RATIO= "%d/%d";
    public  static final String FORMAT_ANSWER_USER_SELECTION= "%d#";
    public  static final String TOKEN_TRUE_SELECT_ANSWER= "1#";
    public  static final String TOKEN_FALSE_SELECT_ANSWER= "0#";
    public  static final String TOKEN_SPLIT_ANSWER_USER_SELECTION= "#";
    public  static final int PROJECT_NORMAL_TYPE = 0;
    public  static final int PROJECT_VOCABULARY_TYPE = 1;
    public  static final int PROJECT_EXAM_MODE = 1;
    public  static final int PROJECT_CHALLENGE_MODE = 2;
    public  static final int PROJECT_CHALLENGE_ANSWERING = 1;
    public  static final int PROJECT_CHALLENGE_ANSWERED = 2;
}
